...
def shopping_cart():
    ...
    for melon_id in cart_items:
        melon_info = melons.get_by_id(melon_id)
        cart_melon_data = cart.get(melon_id, {})
        if cart_melon_data:
            cart_melon_data['quantity'] += 1
            cart_melon_data['total'] += melon_info.price
        else:
            cart_melon_data = {'image': melon_info.image_url,
                               'name': melon_info.common_name,
                               'quantity': 1,
                               'unit price': melon_info.price,
                               'total': melon_info.price}
        cart[melon_id] = cart_melon_data
        total += melon_info.price

    if not cart_items:
        flash("No melons in your cart! Buy melons^_^")
